...
static int presetline=0;
//static int c[12000][100][100];   // 记录图的两点间路径长度
static double maxtest1=-10;
//[][][]用来记录数值上限用于限定开辟空间的大小
static int cardata[110000][10]={0};//[车辆数][属性]
static int roaddata[10000][10]={0};//[道路数][属性]
static int crossdata[10000][10]={0};//[路口数][属性]
static int presetdata[60000][50]={0};//[车辆数][属性]
...
static int PresetIDToIndex[110000];
static int record[12000]={0};
static int frejudge=-1;//From 1 To 2
static int c1;
static int c2=2000;
static double c3=3000;//调参
...
void presetin(string nn)
{
    string str;
    ifstream presetfile;
    presetfile.open(nn);
    getline(presetfile,str);
    if(presetfile.is_open())
    {
        presetline=1;//数据从第1行开始存放
        while(!presetfile.eof())
        {
            ...
            while (i < str.length())
            {
                ...
                {
                    presetdata[presetline][point]=sum;
                    if(point==1)
                    {
                        prepoint=cardata[CarIDToIndex[sum]][2];
                        PresetIDToIndex[presetdata[presetline][point]]=presetline;
                    }
                    else if(point!=2)
                    {
                        if (prepoint==roaddata[RoadIDToIndex[sum]][5])
                            frejudge=1;
                        else if(prepoint==roaddata[RoadIDToIndex[sum]][6])
                            frejudge=2;
                        if(roaddata[RoadIDToIndex[sum]][5]==prepoint)
                            prepoint=roaddata[RoadIDToIndex[sum]][6];
                        else
                            prepoint=roaddata[RoadIDToIndex[sum]][5];
        frequency[int((presetdata[presetline][2]+3)/c4)][RoadIDToIndex[sum]][frejudge]+=1;
                    }
            ...
            record[presetdata[presetline][2]]+=1;
            ++presetline;
            //提取数字end
...
{//car road cross answer
   ...
   char* presetargv=argv[4];
//
    //     string carargv="/Users/apple/Desktop/软件精英挑战赛/复赛/2-map-training-1/car.txt";
    //     string roadargv="/Users/apple/Desktop/软件精英挑战赛/复赛/2-map-training-1/road.txt";
    //     string crossargv="/Users/apple/Desktop/软件精英挑战赛/复赛/2-map-training-1/cross.txt";
    // string presetargv="/Users/apple/Desktop/软件精英挑战赛/复赛/2-map-training-1/presetAnswer.txt";
    crossin(crossargv);
    roadin(roadargv);
    carin(carargv);
    presetin(presetargv);
...
//    cout<<frequency<<endl;
    ofstream outfile;
   char* answer=argv[5];
        // string answer="/Users/apple/Desktop/软件精英挑战赛/复赛/2-map-training-1/answer.txt";
    outfile.open(answer);
    for(int i=1;i<=carline;i++)
    {
        if(cardata[i][7]==1)
...
//    for(int i=1;i<=presetline;i++)
//    {
//        outfile<<"("<<presetdata[i][1]<<","<<presetdata[i][2]<<",";
//        for(int j=1;;j++)
//        {
//            if(CarRoadID[i][j]==0)
//                break;
